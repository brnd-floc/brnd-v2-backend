...
export class PodiumService implements OnModuleInit {
  ...
  /**
   * Check if a wallet has sufficient BRND balance to claim a podium
   * Returns the balance in wei and whether it's sufficient
   */
  async checkBrndBalance(
    walletAddress: string,
  ): Promise<{ balanceWei: bigint; requiredWei: bigint; sufficient: boolean }> {
    ...
    const requiredWei = this.BASE_PRICE;
